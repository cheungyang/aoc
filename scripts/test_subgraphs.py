...
def print_state_summary(stage: str, state: dict):
    print(f"\n📊 --- [State at {stage}] ---")
    keys_to_print = [
        "topic", "project_path", "output_path", "image_path", "image_prompt",
...
async def run_ideation_test(topic: str, project_path: str, output_path: str):
...
async def run_video_production_test(topic: str, project_path: str, output_path: str, use_existing: bool = True):
    ...
    if use_existing and not (raw_video_path and os.path.exists(raw_video_path)):
        import glob
        cands = glob.glob(os.path.join(output_path, "*.mp4"))
        if cands:
...
async def run_copywriting_test(topic: str, project_path: str, output_path: str):
...
async def run_node_remix_directly(topic: str, project_path: str, output_path: str):
    ...
    import glob
    raw_cands = glob.glob(os.path.join(output_path, f"{topic}*.mp4"))
    raw_video = raw_cands[0] if raw_cands else ""
    video_plot = _resolve_asset_path(output_path, topic, "video_plot", next_version=False)
...
async def main():
    ...
    parser.add_argument("--project-path", default="pkm/wiki/software/ayla-first-words", help="Project directory.")
    ...
    output_path = normalize_project_path(os.path.join(project_path, "words", args.topic))
    ...
    if args.subgraph == "remix_node":
        await run_node_remix_directly(args.topic, project_path, output_path)
    elif args.subgraph == "ideation":
        await run_ideation_test(args.topic, project_path, output_path)
    elif args.subgraph == "video":
        await run_video_production_test(args.topic, project_path, output_path, use_existing=args.use_existing)
    elif args.subgraph == "copywriting":
        await run_copywriting_test(args.topic, project_path, output_path)
