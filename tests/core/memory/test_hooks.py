...
class TestHooks(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_hook_pre_message(self, mock_manager):
        await hooks.hook_pre_message("session1", "user", "hello")
        mock_manager.append_message.assert_called_once_with("session1", "user", "hello")
    ...
    async def test_hook_pre_message_new(self, mock_manager):
        ...
        await hooks.hook_pre_message("session1", "user", "[new]")
        mock_manager.append_message.assert_called_once_with("session1", "user", "[new]")

    @patch('core.memory.session_message_hook.manager')
    async def test_hook_post_message(self, mock_manager):
        await hooks.hook_post_message("session1", "ai", "Reply text")
        mock_manager.append_message.assert_called_once_with("session1", "ai", "Reply text")
