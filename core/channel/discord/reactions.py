...
class ReactionCallbackHandler(AsyncCallbackHandler):
    ...
    async def _add_reaction_safe(self, emoji: str):
        ...
        message_loop = getattr(
            getattr(self.message, "_state", None), "loop", None
        )
        if (
            isinstance(message_loop, asyncio.AbstractEventLoop)
            and message_loop.is_running()
        ):
            try:
            ...
            if current_loop == message_loop:
                ...
                asyncio.run_coroutine_threadsafe(
                    self.message.add_reaction(emoji), message_loop
                )
        else:
    ...
    async def on_tool_start(
        ...
        if tool_name == "graph_call":
            try:
                ...
                graph_name = (
                    args.get("graph_name")
                    or args.get("subgraph_name")
                    or args.get("name")
                )
                if graph_name and graph_name != "main":
                    try:
                        config = self.graphs_loader.get_graph_config(
                            graph_name
                        )
