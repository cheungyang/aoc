...
class CommandHandler:
    ...
    async def handle_command(
        ...
        if command == "[new]":
            await self._handle_new(session)
            return True
        elif command == "[newall]":
            await self._handle_newall(session)
            return True
        elif command == "[restart]":
            await self._handle_restart(session)
            return True
        elif command in ("[compact]", "[summarize]"):
            await self._handle_compact(session)
            return True
    ...
    async def _handle_compact(self, session: ExecutionContext):
        ...
        from core.runtime.context_pruner import ContextPruner, estimate_total_tokens
        ...
        pruned = await pruner.aauto_prune_session(session, force=True)
